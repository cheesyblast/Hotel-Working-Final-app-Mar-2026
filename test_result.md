...
agent_communication:
  - agent: "testing"
    message: "CRITICAL ISSUES TESTING COMPLETED - MIXED RESULTS: Completed comprehensive testing of the two critical user-reported issues. RESULTS: (1) ❌ ADVANCE PAYMENT BALANCE UPDATE - CRITICAL BUG FOUND: Double counting issue in advance payment system - payments recorded both as daily sales AND income records, causing balance to increase by 2x the expected amount. (2) ✅ DATE EXTENSION FOR CHECKED-IN BOOKINGS - WORKING CORRECTLY: System properly allows date extensions for checked-in bookings as designed. (3) ✅ VALIDATION RULES - WORKING CORRECTLY: All validation rules for checked-in bookings working properly (prevents check-in date changes, checkout date shortening, and room changes). URGENT ACTION REQUIRED: Fix the double counting bug in /app/backend/server.py advance-payment endpoint (lines 2065-2096) - advance payments should only be recorded in ONE place, not both daily_sales AND incomes collections."
  - agent: "testing"
