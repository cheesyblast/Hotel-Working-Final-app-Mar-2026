...
agent_communication:
  - agent: "main"
    message: "Enhanced Dashboard UI with two major improvements as requested by the user: 1) Replaced the Actions column in Recent Upcoming Bookings table with a cleaner dropdown menu approach - added three-dots button that reveals Check-in, Edit, and Cancel options with proper icons and hover effects. Implemented dropdown state management with click-outside-to-close functionality for better UX. 2) Added real-time 'Booked' room status detection for rooms that have bookings scheduled for the current day but aren't checked in yet. Booked rooms display with orange styling (orange background, border, and 🟠 icon) and show 'Check-in today' message. This provides better visual distinction between Available (green), Occupied (red), and Booked (orange) statuses. Ready for testing the enhanced Dashboard UI functionality."
  - agent: "testing"
