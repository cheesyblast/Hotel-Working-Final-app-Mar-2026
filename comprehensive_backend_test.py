...
def test_error_handling_scenarios():
    ...
    error_tests = [
        # Invalid room ID
        ("GET", "/rooms/invalid-room-id", 404, "Invalid Room ID"),
        
        # Invalid booking ID for cancellation
        ("POST", "/cancel/invalid-booking-id", 404, "Invalid Booking Cancellation"),
        
        # Invalid customer ID for checkout
        ("POST", "/checkout", {"customer_id": "invalid-id"}, 404, "Invalid Customer Checkout"),
        
        # Invalid date format for room availability
        ("GET", "/rooms/availability/check?check_in_date=invalid&check_out_date=invalid", 400, "Invalid Date Format"),
    ]
    
    passed_error_tests = 0
    
    for method, endpoint, expected_status_or_data, test_name in error_tests:
        try:
            if method == "GET":
                response = requests.get(f"{API_BASE}{endpoint}", timeout=10)
                expected_status = expected_status_or_data
            else:
                response = requests.post(f"{API_BASE}{endpoint}", json=expected_status_or_data, timeout=10)
                expected_status = 404 if "Invalid" in test_name else 400
            
            if response.status_code == expected_status:
                print(f"  ✅ {test_name}: Correctly returned {response.status_code}")
    ...
    if passed_error_tests >= len(error_tests) * 0.75:  # At least 75% should pass
        add_test_result("Error Handling", True, f"{passed_error_tests}/{len(error_tests)} tests passed")
        return True
    else:
        add_test_result("Error Handling", False, f"Only {passed_error_tests}/{len(error_tests)} tests passed")
        return False
